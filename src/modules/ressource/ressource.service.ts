...
export class ResourceService {
  ...
  async set(params) {
    ...
    const r = await this.ResourceModel.findOne({ where: { name } });
    if (r) {
    ...
    const res = await this.ResourceModel.query(
      `select max(orderId) as max_order from tb_resource`,
    );
    let { max_order } = res[0];
  ...
  async query(params) {
    ...
    const recourceType = await this.ResourceTypeModel.find({
      where: { id: In(resourceIds) },
    });
    rows.forEach(
      (t: any) =>
        (t.resourceName = recourceType.find((k) => k.id == t.resourceId)[
          'name'
        ]),
    );
